...
fn main() {
    ...
    let elves : Vec<Elf> = buffer.lines().into_iter()
        .filter_map(|x|x.ok())
        .group_by(|elem| !elem.is_empty() )
        .into_iter()
        .filter(|(key, _)| *key )
        .map(|(_ ,group)| Elf {calories : group.filter_map(|s| s.parse::<u32>().ok()).collect() })
        .collect();
    let top = elves.iter().map(|elf| elf.total()).max();
    let top_3: u32 = elves.iter().map(|elf| elf.total()).sorted().rev().take(3).sum();
